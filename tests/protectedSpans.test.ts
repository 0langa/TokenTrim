...
describe('protected spans', () => {
  it('protects and restores representative span types', () => {
    ...
    const run = protectSpans(input, [
      'markdown-heading','markdown-table','inline-code','fenced-code','json-block','yaml-toml','url','file-path','cli-command','env-var','api-placeholder','number-unit','email','quoted-string',
    ]);
    expect(run.text).not.toContain('https://example.com');
    expect(run.text).not.toContain('{"k":1}');
