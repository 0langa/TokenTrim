...
export function TransformStatsPanel({ result }: Props) {
  ...
  if (transformStats.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-32 text-slate-500 text-sm gap-1">
        <span>No transforms applied</span>
        <span className="text-xs">at <span className="font-mono">{result.mode}</span> mode with this use case</span>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 text-xs space-y-3 overflow-auto">
      {/* Summary */}
      <div className="flex flex-wrap gap-x-5 gap-y-1 pb-3 border-b border-slate-800 text-slate-400">
      ...
      <table className="w-full text-left">
        ...
        <tbody>
          {sorted.map((s) => (
            <tr key={s.transformId} className="border-t border-slate-800/60 hover:bg-slate-800/20">
              <td className="py-1.5 font-mono text-slate-300 pr-4">{s.transformId}</td>
              <td className="py-1.5 text-right text-slate-400 pr-4">{s.replacements}</td>
              <td className="py-1.5 text-right text-slate-400 pr-4">
      ...
      {result.rejectedTransforms.length > 0 && (
        <div className="pt-2 border-t border-slate-800">
          <div className="text-slate-400 mb-1.5">Rejected — safety check failed, not applied to output</div>
      ...
      {result.report.riskEvents.length > 0 && (
        <div className="pt-2 border-t border-slate-800">
          <div className="text-slate-400 mb-1.5">Sample changes</div>
